...
def bootstrap(debuts,d,f,dR,dQ,a,B_tilde0,r,graph=False):
    ...
    Rbtot=R[:d]+Rb
    Qbtot=Q[:d]+Qb
    x0=estimation_a_B_tilde(a,B_tilde0,Rbtot,Qbtot,d,d+len(Rb)-1)
    B_tilde=x0['x'][0]
    if graph is True:
...
def Bootstrap(debuts,d,f,R,Q,a,B_tilde0,r,B):
    ...
    for b in range(0,B):
        print(b)
        res=bootstrap(debuts,d,f,dR,dQ,a,B_tilde0,r)
        v_a.append(res[0])
        v_mu.append(res[1])
...
##r=0.1
##
##debuts=[0,45,281,380,470,604,817] 
###graphique_blocs(debuts,d,f,P,len(debuts),B_tilde0)
##
##B=100
##v_a,v_mu,v_sigma2,v_CT=Bootstrap(debuts,d,f,R,Q,a,B_tilde0,r,B)
##IC_bootstrap(v_a,v_mu,v_sigma2,v_CT,90)
##IC_bootstrap(v_a,v_mu,v_sigma2,v_CT,95)
##
### on en fait un avec un graphique
####dR=exp(diff(log(R[d:f+1])))
####dQ=exp(diff(log(Q[d:f+1])))
