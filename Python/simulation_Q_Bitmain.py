...
def simulationrevenues(a,B_tilde,Q_sim,price,d_price,d):

    T=len(Q_sim) # length of study period
    Rev_sim=[]
    Rev_sim.append(0)
    for t in range(1,T):
        Rev_sim.append(Rev_sim[-1]+(max(Q_sim[t]-Q_sim[t-1],0)*price*exp(-(t+d-d_price))
    return Rev_sim # returns simulated Q and P
